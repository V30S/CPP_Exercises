...
#define ANSWER_3 \
    true // car quand on detruit values les pointeurs ne sont pas detruits. il faut faire un delete sur chaque
         // pointeur
...
// Est-ce que l'utilisation de la classe ci-dessus est susceptible de créer des problèmes de mémoire (fuite,
// double-déallocation, etc.) ?
#define ANSWER_4 false // car on détruit les pointeurs ownan de values dans le destructeur
...
// double-déallocation, etc.) ?
#define ANSWER_5 \
    true // car on ne doit pas utiliser delete sur les pointeurs observant dans le destructeur. Quand le
         // vecteur est detruit, les pointeurs sont detruits automatiquement c'est donc redondant.
...
#define ANSWER_22 false // override est optionnel mais c'est une bonne pratique
// 23. Si Mere::fct() est virtuelle pure, il est impossible d'instancier la classe Mere.
#define ANSWER_23 true // car on ne peut pas instancier une classe avec une fonction virtuelle pure
// 24. On utilise le mot-clef "pure virtual" pour indiquer qu'une fonction-membre est virtuelle pure.
#define ANSWER_24 false // c'est = 0
// 25. S'il est impossible d'instancier la classe Mere, le code à la ligne 1 ci-dessus ne compile pas.
#define ANSWER_25 false // car on peut instancier une classe fille avec un pointeur de type mere
